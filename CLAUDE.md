...
이 저장소는 2개 미션 중 "[미션2] 프로젝트 개발"에 해당합니다. 미션1(PoC, 별도 저장소)에서는 MVC 스켈레톤,
데이터 영속성, 데이터 모니터링 도구, Dummy 데이터 생성 도구를 각각 독립적으로 검증했고, 이 저장소는 그 요소들을
아래에 설명된 전체 시스템으로 통합하는 단계입니다. 명세에 따르면 이번 미션의 평가 주안점은 CLAUDE.md/PRD.md 등
문서 관리, Harness 도입, Test, CleanCode, Commit 이력이므로 이를 "있으면 좋은 것"이 아니라 실제 평가 기준으로
취급해야 합니다.
...
이 프로젝트가 이어받는 미션1 PoC 요구사항(MVC 스켈레톤, CRUD를 포함한 영속성 저장, 데이터 모니터링 도구,
Dummy 데이터 생성 도구)을 고려하여, Model/Controller/View를 명확히 분리한 구조로 설계합니다:
