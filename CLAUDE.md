...
이 저장소는 `docs/PLAN.md`의 Phase 0~7(뼈대, 시료 관리, 시료 주문, 승인/거절, 생산 라인, 출고 처리,
모니터링, 마무리)까지 구현이 끝난 상태입니다 — 메인 메뉴 6개 항목이 모두 실제 기능으로 동작하며,
`app/model`, `app/persistence`, `app/controller`, `app/view`, `app/generator`(더미 데이터 생성),
`main.py`, `generate_dummy_data.py`, `README.md`, `tests/`(pytest, 각 Phase별 단위 테스트 + 전체 흐름
통합 테스트)가 존재합니다. 이 프로젝트는 Python 으로 구현합니다.
...
- [`docs/design/`](docs/design) — `docs/PLAN.md`의 각 Phase가 위임하는 상세 설계 문서(클래스/파일 단위
  구조, PoC 간 설계 결정 차이 조정, 완료 기준 매핑). Phase를 구현하기 전에 해당 Phase의 설계 문서가 있는지
  먼저 확인한다 (예: [`phase0.md`](docs/design/phase0.md), [`phase1.md`](docs/design/phase1.md)).
...
- **시료(Sample)**: 카탈로그의 기본 단위. 속성은 시료 ID, 이름, 평균 생산시간(개당), 수율(정상품 수 / 총 생산량,
  ...
  > **수율의 실제 구현 방식(수업 규칙)**: 수율의 정의 자체는 "정상품 수 / 총 생산량"이지만, 이 프로젝트는
  > 불량품/정상품을 실제로 나누어 추적하지 않습니다. 수율은 오직 **"부족분을 채우려면 얼마나 더 많이
  > 생산 지시를 내려야 하는가"**(`ceil(부족분 / 수율)`)를 정하는 데만 쓰이고, 일단 그 수량만큼 생산이
  > 끝나면 **생산된 수량 전체를 정상품으로 간주**해 재고에 반영합니다(불량품으로 인한 손실을 다시
  > 차감하지 않음). 개념적으로 다소 어색하게 느껴질 수 있으나, 이 프로젝트의 의도된 단순화이며
  > 상세 근거는 [`docs/FEATURES/05-production-line.md`](docs/FEATURES/05-production-line.md) "재고 반영
  > 정책" 절을 참고하세요.
