"""Sample 전용 JSON 리포지토리. `data/samples.json`을 기본 저장 경로로 사용한다."""
...
DEFAULT_PATH = "data/samples.json"
...
class SampleRepository(JsonRepository[Sample]):
    ...
    def _max_existing_seq(self) -> int:
        max_seq = 0
        for sample in self._data.values():
            match = _ID_PATTERN.match(sample.sample_id)
            if match:
